...
#+begin_src python :results file :return figfile
  ...
  gg_init = (
      models.Gaussian1D(0.1, -80.0, sig, fixed={"stddev": False})
      + models.Gaussian1D(0.05, -60.0, 1.2*sig, fixed={"stddev": False})
  ...
  for g in gg_fit_n[:-1]:
      ...
      axes[0].plot([g.mean.value]*2, 0.8 + 0.1*np.array([-1, 1]),
              lw=1.5, color=c_n)
  ...
  faint = dict(lw=0.5, ls="--", alpha=0.5, color="k")
  ...
  axes[-1].legend(fontsize="x-small").set_title("(data - model)", prop={"size": "x-small"})
