...
+ [X] Vertical Slit F
  ...
  + y = +6 and above
    + Two similar strength components at -34 and -51
    + => Vc +/- dV = -42.5 +/- 8.5
+ [ ] Vertical Slits G, H
  + y = -8.5 \to -6
...
+ It is co-extensive with [N II] complexes, but may not be exactly the same
...
+ Diagonal slit U
  ...
  + The -60 km/s component (s = -1 \to +1) may be the [O III] equivalent of the SW Faint Blue complex
    + If that is true, then we can associate some particular knots with that complex, which means that we can get proper motions
