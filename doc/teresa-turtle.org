...
:PROPERTIES:
:ID:       2EEF89D6-A11B-446F-9C08-49194B9754C0
:END:
...
Another version that is shifted by 0.15 pix in [N II]
...
#+begin_src python
  ...
  for line in "nii",:
      ...
      fits.PrimaryHDU(header=hvx.header, data=Vr).writeto(
          f"../propermotions/{line}/{line}-Vr-win-SHIFT.fits", overwrite=True,
      )
      fits.PrimaryHDU(header=hvx.header, data=V).writeto(
          f"../propermotions/{line}/{line}-V-win-SHIFT.fits", overwrite=True,
      )
      fits.PrimaryHDU(header=hvx.header, data=Vth).writeto(
          f"../propermotions/{line}/{line}-Vth-win-SHIFT.fits", overwrite=True,
      )
      fits.PrimaryHDU(header=hvx.header, data=theta).writeto(
          f"../propermotions/{line}/{line}-theta-win-SHIFT.fits", overwrite=True,
      )
      fits.PrimaryHDU(header=hvx.header, data=R).writeto(
          f"../propermotions/{line}/{line}-R-win-SHIFT.fits", overwrite=True,
      )
      fits.PrimaryHDU(header=hvx.header, data=PA).writeto(
          f"../propermotions/{line}/{line}-PA-win-SHIFT.fits", overwrite=True,
      )
#+end_src
...
**** [2020-11-30 Mon] Systematic uncertainties in proper motion
:PROPERTIES:
:ID:       135E0BA9-DDC0-4B7B-B480-D7F13C029255
:END:
+ Compare NW knot and SE knot
+ Translation from PM to V_los
  + 95 km/s = 10 mas/yr
+ NW knot
  + R_proj = 8 +/- 1 arcsec
  + V_pos = 11 +/- 8 km/s
    + 1.2 mas/yr
  + t = 6700 (+40,000 -3500) yr 
+ SE knot
  + R_proj = 6 +/- 1 arcsec
  + V_pos = 39 +/- 10
    + 4.1 mas/yr
  + t = 1500 +/- 400 yr
+ Difference between them is 2.9 mas/yr
  + Half that is 1.5 mas/yr
  + So shifting by that would make the timescales the same
  + Or would it? See below


***** Remeasuring proper motions with different shifts
+ These are the values with the original shifts that are in the paper
  |         |   Vr |   Vth |
  | SE knot | 0.48 | -0.07 |
  | NW knot | 0.12 |  0.16 |
  | N knot  | 0.66 |  0.08 |
  | Shell N | 0.59 |  0.16 |
  | Shell S | 0.46 |  0.02 |
  + Note that the SW knot has a higher Vr here than we have in the paper
  + Not sure why ....
+ And these are the values with the extra (0.1, 0.1) shift
  |         |   Vr |   Vth |
  | SE knot | 0.35 | -0.08 |
  | NW knot | 0.25 |  0.17 |
  | N knot  | 0.78 |  0.03 |
  | Shell N | 0.66 |  0.05 |
  | Shell S | 0.32 |  0.03 |
+ Compare with [O III] for the Shell
  |         |   Vr |   Vth |
  | SE knot | 0.33 | -0.07 |
  | NW knot |  0.4 |  0.05 |
  | N knot  | 1.08 |  0.03 |
  | Shell N | 0.56 | -0.04 |
  | Shell S | 0.57 |  0.21 |
+ 1 pix in 10 years is 100 / 10 = 10 mas/yr
  + So 0.14 pix is 1.4 mas/yr
+ So, even with the hypot(0.1, 0.1) = 0.14 pix shift, we still get a difference between the two sides
  + SE knot: t = 1000 6 +/- 1 / 10 0.35 = 1700 +/- 300
  + NW knot: t = 1000 8 +/- 1 / 10 0.25 = 3200 +/- 400.
***** DONE [#A] Repeat more carefully but including all the knot regions
CLOSED: [2020-12-01 Tue 19:05]
:PROPERTIES:
:EXPORT_OPTIONS: toc:nil num:nil
:EXPORT_FILE_NAME: turtle-pm-check
:END:
+ The PM values that we use in the paper are from the [[id:AC5273A6-804B-4270-A3F2-07A2EB0A1393][Table of 3D velocity organized by axis]]
+ These are in pixels, so need to be multiplied by 10 to get mas/yr or by 95 to get km/s
+ We will repeat those here, but then do the measurements again on both the standard and shifted PM maps
+ We will use the maps of R and PA to choose the regions to measure
+ Note that I changed the definition of SE Blue a little to make it more reasonable
  + It is now the region closer to the star than the SE knot, so as not to include any shell emission
+ And also N(E) Red 
  + I expanded it to 2 arcsec radius and moved it to cover more of the complex (previously, it had covered just the far eastern tip)
+ Columns:
  + PM_r (PAPER) :: This is what we currently have in the paper
  + PM_r (BEST) :: New measurements, but using the same alignment of the two epochs as we used before
  + PM_r (SHIFT) :: New measurements, now with the second epoch shifted by 0.15 pixels along PA315
  + PM_th :: The non-radial component of proper motion.  This is always small except for
    1. SW Faint Blue, which is noisy
    2. NW knot.   Not sure why this has non-radial component
+ There are some small changes between PM_r(BEST) and PM_r(PAPER), but these are all within the estimated uncertainties
| System           |  PA | Axis | R_proj  | PM_r (PAPER)  | PM_r (BEST)   | PM_r (SHIFT)  | PM_th           | V_r (BEST)  |
|------------------+-----+------+---------+---------------+---------------+---------------+-----------------+-------------|
| SW Red complex   | 250 | E+   | 7 +/- 2 | 0.19 +/- 0.03 | 0.18 +/- 0.04 | 0.24 +/- 0.05 | 0.02 +/- 0.05   | 17. +/- 4.  |
| N(E) Red complex |   0 | E+   | 4 +/- 2 | 0.05 +/- 0.04 | 0.04 +/- 0.06 | 0.14 +/- 0.04 | 0.02 +/- 0.05   | 4. +/- 6.   |
| NW knot ([N II]) | 310 | E+   | 8 +/- 1 | 0.12 +/- 0.08 | 0.08 +/- 0.06 | 0.22 +/- 0.06 | 0.15 +/- 0.03   | 8. +/- 6.   |
| N(W) Red complex | 300 | E+   | 6 +/- 2 | 0.10 +/- 0.04 | 0.07 +/- 0.04 | 0.20 +/- 0.04 | 0.01 +/- 0.05   | 7. +/- 4.   |
| NE Blue complex  |  60 | E-   | 5 +/- 1 | 0.24 +/- 0.05 | 0.25 +/- 0.03 | 0.21 +/- 0.03 | -0.02 +/- 0.003 | 24. +/- 3.  |
| SW Faint Blue    | 225 | E-   | 7 +/- 1 | 0.27 +/- 0.04 | 0.18 +/- 0.10 | 0.18 +/- 0.09 | 0.15 +/- 0.03   | 17. +/- 10. |
| SE knot          | 135 | E-   | 5 +/- 1 | 0.41 +/- 0.10 | 0.44 +/- 0.03 | 0.30 +/- 0.04 | -0.09 +/- 0.06  | 42. +/- 3.  |
| SE Blue complex  | 135 | E-   | 3 +/- 2 | 0.41 +/- 0.10 | 0.34 +/- 0.12 | 0.21 +/- 0.11 | -0.03 +/- 0.08  | 32. +/- 11. |
#+TBLFM: $9=$6 95;f0

****** Red knots
+ Calculate timescales per feature/system and also for the average of all red knots
+ Remove some of the columns that we aren't using, so we have room for the time columns
+ Times are in kyr
+ The BEST columns are for our preferred solution for the alignment
+ The SHIFT columns are with the worst-case shift of 0.15 pix at PA=315
| System           |  PA | Axis | R_proj | PM_r (BEST)     | PM_r (SHIFT)    | t (BEST)        | t (SHIFT)     |
|------------------+-----+------+--------+-----------------+-----------------+-----------------+---------------|
| SW Red complex   | 250 | E+   |      7 | 0.18 +/- 0.04   | 0.24 +/- 0.05   | 3.89 +/- 0.86   | 2.92 +/- 0.61 |
| N(E) Red complex |   0 | E+   |      4 | 0.04 +/- 0.06   | 0.14 +/- 0.04   | 10.00 +/- 15.00 | 2.86 +/- 0.82 |
| NW knot ([N II]) | 310 | E+   |      8 | 0.08 +/- 0.06   | 0.22 +/- 0.06   | 10.00 +/- 7.50  | 3.64 +/- 0.99 |
| N(W) Red complex | 300 | E+   |      6 | 0.07 +/- 0.04   | 0.20 +/- 0.04   | 8.57 +/- 4.90   | 3.00 +/- 0.60 |
|------------------+-----+------+--------+-----------------+-----------------+-----------------+---------------|
|                  |     |      |  6.250 | 0.105 +/- 0.025 | 0.192 +/- 0.025 | 5.95 +/- 1.42   | 3.26 +/- 0.42 |
#+TBLFM: $7=$4 / 10  $5; f2::$8=$4 / 10 $6 ; f2::@6$4=vmean(@I..@II) ; f3::@6$5=vmean(@I..@II) +/- 0.025 ; f3::@6$6=vmean(@I..@II) +/- 0.025 ; f3
+ To calculate the means (last row), we ignore the +/- on the positions since these are region sizes, not uncertainties
+ The +/- of the PM is taken as mean of +/- on individual values, divided by sqrt(N)
****** Blue knots
+ Repeat but for the blue knots
| System          |  PA | Axis | R_proj | PM_r (BEST)     | PM_r (SHIFT)    | t (BEST)      | t (SHIFT)     |
|-----------------+-----+------+--------+-----------------+-----------------+---------------+---------------|
| NE Blue complex |  60 | E-   |      5 | 0.25 +/- 0.03   | 0.21 +/- 0.03   | 2.00 +/- 0.24 | 2.38 +/- 0.34 |
| SW Faint Blue   | 225 | E-   |      7 | 0.18 +/- 0.10   | 0.18 +/- 0.09   | 3.89 +/- 2.16 | 3.89 +/- 1.94 |
| SE knot         | 135 | E-   |      5 | 0.44 +/- 0.03   | 0.30 +/- 0.04   | 1.14 +/- 0.08 | 1.67 +/- 0.22 |
| SE Blue complex | 135 | E-   |      3 | 0.34 +/- 0.12   | 0.21 +/- 0.11   | 0.88 +/- 0.31 | 1.43 +/- 0.75 |
|-----------------+-----+------+--------+-----------------+-----------------+---------------+---------------|
|                 |     |      |      5 | 0.338 +/- 0.035 | 0.237 +/- 0.030 | 1.48 +/- 0.15 | 2.11 +/- 0.27 |
#+TBLFM: $7=$4 / 10  $5; f2::$8=$4 / 10 $6 ; f2::@6$4=vmean(@I..@II) ; f3::@6$5=vmean(@I..@II) +/- 0.035 ; f3::@6$6=vmean(@I..@II) +/- 0.03 ; f3
****** Difference between red and blue knots
+ With the BEST alignment we have
  + Red knots: t = 6.7 +/- 1.6 kyr (total range 4 \to 10)
  + Blue knots: t = 1.6 +/- 0.2 kyr (total range 1 \to 4)
  + Mean difference: dt = 5.1 +/- 1.6 => larger than 3 sigma
  + No time overlap of ejections
+ With the SHIFT alignment
  + Red knots: t = 3.7 +/- 0.5 kyr (total range 3 \to 4)
  + Blue knots: t = 2.3 +/- 0.3 kyr (total range 1 \to 4)
  + Mean difference: dt = 1.4 +/- 0.6 kyr => larger than 2 sigma
  + Possible time overlap of ejections
    + SW Faint Blue is contemporaneous with the Red knots
      + *but* this is also the least certain measurement
    + The red knots may easily all have been ejected at the same time (roughly 3500 yrs ago)
    + The blue knots are /marginally/ consistent with a common ejection time of about 2000 yrs ago
      
***** EARLIER VERSION of Taking averages of the red and blue knot timescales
+ For individual objects, the random errors in the proper motions are quite large, but these can be reduced by averaging all the red knots and all of the blue knots
+ Hopefully, this will reduce the fractional uncertainty by a bit
+ Then the dominant term that remains will be the systematic uncertainty, which is not reduced by averaging
+ Red knots: E+
  | Knot    |          V |  eV |           R |  eR |
  |---------+------------+-----+-------------+-----|
  | NW knot |         11 |   8 |           8 |   1 |
  ...
  | NE red  |          5 |   4 |           4 |   1 |
  |---------+------------+-----+-------------+-----|
  |         | 11 +/- 2.7 | 2.4 | 6.3 +/- 0.9 | 0.8 |
  #+TBLFM: @6$2=vmeane(@I..@II);f1::@6$3=vmean(@I..@II)/sqrt(4);f1::@6$4=vmeane(@I..@II);f1::@6$5=vmean(@I..@II)/sqrt(4);f1
+ Blue knots: E-
  | Knot    |            V |  eV |           R |  eR |
  |---------+--------------+-----+-------------+-----|
  | SE knot |           39 |  10 |           6 |   1 |
  ...
  | SW Fb   |           26 |   4 |           7 |   1 |
  |---------+--------------+-----+-------------+-----|
  |         | 31.8 +/- 4.2 | 3.6 | 5.8 +/- 0.5 | 0.5 |
  #+TBLFM: @6$2=vmeane(@I..@II);f1::@6$3=vmean(@I..@II)/sqrt(4);f1::@6$4=vmeane(@I..@II);f1::@6$5=vmean(@I..@II)/sqrt(4);f1
+ In these tables, the final row has the following:
  + V column :: Sample mean POS velocity +/- sample dispersion
  + eV column :: s.e.m of POS velocity (average random error divided by sqrt of number of samples)
  + R column :: Sample mean projected radius +/- sample dispersion
  + eR column :: s.e.m. of R
+ One could argue that the eR should be ignored, since these are really the size of the knots or complexes, and the actual error in determining the radius is very small
+ In that case, we estimate the timescale as R/V with relative error given by (eV/V)
  + But with conversion of V from km/s to to mas/yr (multiply by 10/95)
+ Mean timescales:
  + Red knots: 6.3 / (10/95) 11.0 = 5.4 kyr
  + Red uncertainty: (2.4/11) 5.441 = 1.2 kyr
  + Blue knots: 5.8 / (10/95) 31.8 = 1.7 kyr
  + Blue uncertainty: (3.6/31.8) 1.882 = 0.2 kyr
+ Final result with random errors:
  + Red: 5.4 +/- 1.2 kyr
  + Blue: 1.7 +/- 0.2 kyr
  + Difference: 3.7 +/- 1.22 => 2.9 sigma significance
+ Effect of systematic errors:
  + Error is 1.5 mas/yr = 14.25 km/s
  + But due to spread of angles, average would be more like 10 km/s
  + So the velocities would be
    + 21.8 Red
    + 21.0 Blue
...
:PROPERTIES:
:ID:       AC5273A6-804B-4270-A3F2-07A2EB0A1393
:END:
...
***** New version after re-measuring proper motions of knots
| System               |  PA | Axis | V_los       | PM_r          | V_pos       | V           | i            | R_proj | R, pc           | t, 1000 yr    |
|----------------------+-----+------+-------------+---------------+-------------+-------------+--------------+--------+-----------------+---------------|
| Inner shell A NNW    | 340 | A+   | +8 +/- 3    | 0.51 +/- 0.07 | 48. +/- 7.  | 49. +/- 7.  | 11. +/- 4.   |      7 | 0.07 +/- 9.7e-4 | 1.4 +/- 0.2   |
| Outer lobe I+/VII+   | 340 | A+   | +28 +/- 2   | 0.86 +/- 0.3  | 82. +/- 29. | 89. +/- 27. | 22. +/- 7.   |     16 | 0.17 +/- 8.5e-3 | 1.9 +/- 0.6   |
| North knot           | 340 | A+   | +24 +/- 1   | 0.63 +/- 0.04 | 60. +/- 4.  | 67. +/- 4.  | 26. +/- 2.   |     17 | 0.19 +/- 3.2e-3 | 2.7 +/- 0.2   |
| Inner shell A SSE    | 160 | A-   | -17 +/- 3   | 0.54 +/- 0.07 | 51. +/- 7.  | 55. +/- 7.  | -22. +/- 4.  |      7 | 0.08 +/- 2.1e-3 | 1.3 +/- 0.2   |
| Outer lobe I-        | 160 | A-   | -32 +/- 1   | 0.75 +/- 0.30 | 71. +/- 29. | 81. +/- 26. | -28. +/- 10. |     15 | 0.17 +/- 0.02   | 2.0 +/- 0.8   |
| Outer lobe VIII-     | 160 | A-   | -10 +/- 1   | 0.71 +/- 0.09 | 67. +/- 9.  | 68. +/- 9.  | -10. +/- 2.  |     23 | 0.23 +/- 1.4e-3 | 3.2 +/- 0.4   |
| Outer lobe II+       | 170 | C+   | +24 +/- 1.5 | 0.61 +/- 0.13 | 58. +/- 12. | 65. +/- 11. | 26. +/- 5.   |     24 | 0.27 +/- 0.01   | 3.9 +/- 0.8   |
| Outer lobe II-       | 355 | C-   | -25 +/- 3   | 0.5 +/- 0.5   | 48. +/- 48. | 57. +/- 41. | -32. +/- 26. |     21 | 0.25 +/- 0.07   | 4.2 +/- 4.2   |
|----------------------+-----+------+-------------+---------------+-------------+-------------+--------------+--------+-----------------+---------------|
| Inner shell B ENE    |  65 | B-   | -25 +/- 3   | 0.33 +/- 0.05 | 31. +/- 5.  | 43. +/- 4.  | -44. +/- 6.  |      4 | 0.06 +/- 5.6e-3 | 1.2 +/- 0.2   |
| NE Blue complex      |  60 | E-   | -51 +/- 1   | 0.25 +/- 0.03 | 24. +/- 3.  | 66. +/- 2.  | -69. +/- 2.  |      5 | 0.14 +/- 0.01   | 2.0 +/- 0.2   |
| IS ENE               |  50 | B-   | -18 +/- 3   | 0.28 +/- 0.16 | 27. +/- 15. | 35. +/- 12. | -39. +/- 16. |      9 | 0.12 +/- 0.03   | 3.2 +/- 1.8   |
| Inner shell B WSW    | 240 | B+   | +18 +/- 3   | 0.31 +/- 0.04 | 29. +/- 4.  | 36. +/- 4.  | 37. +/- 6.   |      5 | 0.06 +/- 4.9e-3 | 1.6 +/- 0.2   |
| IS WSW               | 260 | B+   | +20 +/- 3   | 0.30 +/- 0.27 | 29. +/- 26. | 38. +/- 20. | 40. +/- 26.  |     12 | 0.16 +/- 0.06   | 4.0 +/- 3.6   |
| SW Red complex       | 250 | E+   | +38 +/- 1   | 0.18 +/- 0.04 | 17. +/- 4.  | 49. +/- 2.  | 70. +/- 4.   |      7 | 0.20 +/- 0.04   | 3.9 +/- 0.9   |
| N(E) Red complex     |  30 | E+   | +25 +/- 2   | 0.04 +/- 0.04 | 4. +/- 4.   | 30. +/- 2.  | 82. +/- 8.   |      4 | 0.29 +/- 0.29   | 10.0 +/- 10.0 |
| SW Faint Blue        | 225 | E-   | -26 +/- 1   | 0.18 +/- 0.10 | 17. +/- 10. | 36. +/- 5.  | -61. +/- 14. |      7 | 0.14 +/- 0.06   | 3.9 +/- 2.2   |
|----------------------+-----+------+-------------+---------------+-------------+-------------+--------------+--------+-----------------+---------------|
| Outer lobe X+        | 285 | D+   | +6 +/- 2    | 0.50 +/- 0.45 | 48. +/- 43. | 49. +/- 43. | 9. +/- 8.    |     19 | 0.19 +/- 4.3e-3 | 3.8 +/- 3.4   |
| Outer lobe IV+       | 315 | D+   | +13 +/- 1   | 0.35 +/- 0.07 | 33. +/- 7.  | 37. +/- 6.  | 25. +/- 5.   |     10 | 0.11 +/- 4.5e-3 | 2.9 +/- 0.6   |
| NW knot ([N II])     | 310 | E+   | +18 +/- 1   | 0.08 +/- 0.06 | 8. +/- 6.   | 23. +/- 2.  | 70. +/- 14.  |      8 | 0.23 +/- 0.16   | 10.0 +/- 7.5  |
| N(W) Red complex     | 300 | E+   | +21 +/- 2   | 0.07 +/- 0.04 | 7. +/- 4.   | 26. +/- 3.  | 74. +/- 9.   |      6 | 0.22 +/- 0.12   | 8.6 +/- 4.9   |
| Outer lobe X-/IV-/V+ | 120 | D-   | -8 +/- 4    | 0.25 +/- 0.15 | 24. +/- 14. | 26. +/- 13. | -22. +/- 15. |     13 | 0.14 +/- 0.01   | 5.2 +/- 3.1   |
| SE Blue complex      | 135 | E-   | -31 +/- 3   | 0.34 +/- 0.12 | 32. +/- 11. | 49. +/- 8.  | -49. +/- 10. |      3 | 0.05 +/- 9.2e-3 | 0.9 +/- 0.3   |
| SE knot              | 135 | E-   | -35 +/- 3   | 0.44 +/- 0.03 | 42. +/- 3.  | 59. +/- 3.  | -45. +/- 3.  |      5 | 0.07 +/- 3.7e-3 | 1.1 +/- 8e-2  |
#+TBLFM: $6=95 $5;f0::$7=sqrt((1.2 ($4))**2 +  ($6)**2) ;f0::$8=arctan(1.2 ($4) / ($6)) ;f0::$10=0.01 $9/cos($8);f2::$11=100 $9 / 1000 $5 ;f1

***** And another new version with the worst-case systematic error applied
+ This is only changed for the knots for now, but we should apply it to everything really
  + Although it would have less effect for the [O III] features, since they tend to have larger PM
| System               |  PA | Axis | V_los       | PM_r          | V_pos       | V           | i            | R_proj | R, pc           | t, 1000 yr  |
|----------------------+-----+------+-------------+---------------+-------------+-------------+--------------+--------+-----------------+-------------|
| Inner shell A NNW    | 340 | A+   | +8 +/- 3    | 0.51 +/- 0.07 | 48. +/- 7.  | 49. +/- 7.  | 11. +/- 4.   |      7 | 0.07 +/- 9.7e-4 | 1.4 +/- 0.2 |
| Outer lobe I+/VII+   | 340 | A+   | +28 +/- 2   | 0.86 +/- 0.3  | 82. +/- 29. | 89. +/- 27. | 22. +/- 7.   |     16 | 0.17 +/- 8.5e-3 | 1.9 +/- 0.6 |
| North knot           | 340 | A+   | +24 +/- 1   | 0.78 +/- 0.04 | 74. +/- 4.  | 79. +/- 4.  | 21. +/- 1.   |     17 | 0.18 +/- 1.2e-3 | 2.2 +/- 0.1 |
| Inner shell A SSE    | 160 | A-   | -17 +/- 3   | 0.54 +/- 0.07 | 51. +/- 7.  | 55. +/- 7.  | -22. +/- 4.  |      7 | 0.08 +/- 2.1e-3 | 1.3 +/- 0.2 |
| Outer lobe I-        | 160 | A-   | -32 +/- 1   | 0.75 +/- 0.30 | 71. +/- 29. | 81. +/- 26. | -28. +/- 10. |     15 | 0.17 +/- 0.02   | 2.0 +/- 0.8 |
| Outer lobe VIII-     | 160 | A-   | -10 +/- 1   | 0.71 +/- 0.09 | 67. +/- 9.  | 68. +/- 9.  | -10. +/- 2.  |     23 | 0.23 +/- 1.4e-3 | 3.2 +/- 0.4 |
| Outer lobe II+       | 170 | C+   | +24 +/- 1.5 | 0.61 +/- 0.13 | 58. +/- 12. | 65. +/- 11. | 26. +/- 5.   |     24 | 0.27 +/- 0.01   | 3.9 +/- 0.8 |
...
|----------------------+-----+------+-------------+---------------+-------------+-------------+--------------+--------+-----------------+-------------|
| Inner shell B ENE    |  65 | B-   | -25 +/- 3   | 0.33 +/- 0.05 | 31. +/- 5.  | 43. +/- 4.  | -44. +/- 6.  |      4 | 0.06 +/- 5.6e-3 | 1.2 +/- 0.2 |
| NE Blue complex      |  60 | E-   | -51 +/- 1   | 0.21 +/- 0.03 | 20. +/- 3.  | 64. +/- 1.  | -72. +/- 3.  |      5 | 0.16 +/- 0.03   | 2.4 +/- 0.3 |
| IS ENE               |  50 | B-   | -18 +/- 3   | 0.28 +/- 0.16 | 27. +/- 15. | 35. +/- 12. | -39. +/- 16. |      9 | 0.12 +/- 0.03   | 3.2 +/- 1.8 |
| Inner shell B WSW    | 240 | B+   | +18 +/- 3   | 0.31 +/- 0.04 | 29. +/- 4.  | 36. +/- 4.  | 37. +/- 6.   |      5 | 0.06 +/- 4.9e-3 | 1.6 +/- 0.2 |
| IS WSW               | 260 | B+   | +20 +/- 3   | 0.30 +/- 0.27 | 29. +/- 26. | 38. +/- 20. | 40. +/- 26.  |     12 | 0.16 +/- 0.06   | 4.0 +/- 3.6 |
| SW Red complex       | 250 | E+   | +38 +/- 1   | 0.24 +/- 0.05 | 23. +/- 5.  | 51. +/- 2.  | 63. +/- 5.   |      7 | 0.15 +/- 0.03   | 2.9 +/- 0.6 |
| N(E) Red complex     |  30 | E+   | +25 +/- 2   | 0.14 +/- 0.04 | 13. +/- 4.  | 33. +/- 3.  | 67. +/- 7.   |      4 | 0.10 +/- 0.03   | 2.9 +/- 0.8 |
| SW Faint Blue        | 225 | E-   | -26 +/- 1   | 0.18 +/- 0.09 | 17. +/- 9.  | 36. +/- 4.  | -61. +/- 13. |      7 | 0.14 +/- 0.06   | 3.9 +/- 1.9 |
|----------------------+-----+------+-------------+---------------+-------------+-------------+--------------+--------+-----------------+-------------|
| Outer lobe X+        | 285 | D+   | +6 +/- 2    | 0.50 +/- 0.45 | 48. +/- 43. | 49. +/- 43. | 9. +/- 8.    |     19 | 0.19 +/- 4.3e-3 | 3.8 +/- 3.4 |
| Outer lobe IV+       | 315 | D+   | +13 +/- 1   | 0.35 +/- 0.07 | 33. +/- 7.  | 37. +/- 6.  | 25. +/- 5.   |     10 | 0.11 +/- 4.5e-3 | 2.9 +/- 0.6 |
| NW knot ([N II])     | 310 | E+   | +18 +/- 1   | 0.22 +/- 0.06 | 21. +/- 6.  | 30. +/- 4.  | 46. +/- 8.   |      8 | 0.12 +/- 0.02   | 3.6 +/- 1.0 |
| N(W) Red complex     | 300 | E+   | +21 +/- 2   | 0.20 +/- 0.04 | 19. +/- 4.  | 32. +/- 3.  | 53. +/- 6.   |      6 | 0.10 +/- 0.01   | 3.0 +/- 0.6 |
| Outer lobe X-/IV-/V+ | 120 | D-   | -8 +/- 4    | 0.25 +/- 0.15 | 24. +/- 14. | 26. +/- 13. | -22. +/- 15. |     13 | 0.14 +/- 0.01   | 5.2 +/- 3.1 |
| SE Blue complex      | 135 | E-   | -31 +/- 3   | 0.21 +/- 0.12 | 20. +/- 11. | 42. +/- 6.  | -62. +/- 13. |      3 | 0.06 +/- 0.03   | 1.4 +/- 0.8 |
| SE knot              | 135 | E-   | -35 +/- 3   | 0.30 +/- 0.03 | 29. +/- 3.  | 51. +/- 3.  | -55. +/- 4.  |      5 | 0.09 +/- 8.7e-3 | 1.7 +/- 0.2 |
#+TBLFM: $6=95 $5;f0::$7=sqrt((1.2 ($4))**2 +  ($6)**2) ;f0::$8=arctan(1.2 ($4) / ($6)) ;f0::$10=0.01 $9/cos($8);f2::$11=100 $9 / 1000 $5 ;f1
...
| NE Blue complex       |    2 |  60 |      | 66. |  2. | 0.14 | 0.01 |  2.0 | 0.2 |
...
| SW Red complex        |    2 | 250 |      | 49. |  2. | 0.20 | 0.04 |  3.9 | 0.9 |
| N(E) Red complex      |    2 |  30 |      | 31. |  2. | 0.15 | 0.15 |  5.0 | 5.0 |
| SW Faint Blue complex |    2 | 225 |      | 36. |  5. | 0.14 | 0.06 |  3.9 | 2.2 |
|-----------------------+------+-----+------+-----+-----+------+------+------+-----|
| Outer lobe X+         |    3 | 285 | D+   | 51. | 43. | 0.19 | 0.02 |  3.7 | 3.3 |
| Outer lobe IV+        |    3 | 315 | D+   | 37. |  6. | 0.11 | 0.01 |  2.9 | 0.6 |
| NW knot ([N II])      |    2 | 310 |      | 23. |  2. | 0.23 | 0.16 | 10.0 | 7.5 |
| N(W) Red complex      |    2 | 300 |      | 26. |  3. | 0.22 | 0.12 |  8.6 | 4.9 |
| Outer lobe X-/IV-/V+  |    3 | 120 | D-   | 26. | 13. | 0.14 | 0.04 |  5.2 | 3.3 |
| SE Blue complex       |    2 | 135 |      | 49. |  8. | 0.05 | 0.01 |  0.9 | 0.3 |
| SE knot               |    2 | 135 |      |  59 |   3 | 0.07 | 0.01 |  1.1 | 0.1 |

And the same for the worst-case PM shift of the knots.
#+name: vel-radius-systems-worst
| System                | Type |  PA | Axis |   V |  eV |    R |   eR |   t |  et |
|-----------------------+------+-----+------+-----+-----+------+------+-----+-----|
| Inner shell A NNW     |    1 | 340 | A^+  | 49. |  7. | 0.07 | 0.01 | 1.4 | 0.2 |
| Outer lobe I+/VII+    |    3 | 340 | A^+  | 89. | 27. | 0.17 | 0.01 | 1.9 | 0.7 |
| North knot            |    2 | 340 | A^+  | 79. |  4. | 0.18 | 0.01 | 2.2 | 0.2 |
| Inner shell A SSE     |    1 | 160 | A^-  | 55. |  7. | 0.08 | 0.01 | 1.2 | 0.2 |
| Outer lobe I-         |    3 | 160 | A^-  | 81. | 26. | 0.17 | 0.03 | 2.0 | 0.8 |
| Outer lobe VIII-      |    3 | 160 | A^-  | 68. |  9. | 0.23 | 0.01 | 3.2 | 0.4 |
| Outer lobe II+        |    3 | 170 | C^+  | 65. | 11. | 0.27 | 0.02 | 3.9 | 0.9 |
...
|-----------------------+------+-----+------+-----+-----+------+------+-----+-----|
| Inner shell B ENE     |    1 |  65 | B^-  | 43. |  4. | 0.06 | 0.01 | 1.2 | 0.4 |
| NE Blue complex       |    2 |  60 |      | 64. |  1. | 0.16 | 0.03 | 2.4 | 0.3 |
| IS ENE                |    1 |  50 | B^-  | 35. | 12. | 0.12 | 0.04 | 3.2 | 2.0 |
| Inner shell B WSW     |    1 | 240 | B^+  | 36. |  4. | 0.06 | 0.01 | 1.5 | 0.4 |
| IS WSW                |    1 | 260 | B^+  | 38. | 20. | 0.16 | 0.06 | 3.8 | 3.5 |
| SW Red complex        |    2 | 250 |      | 51. |  2. | 0.15 | 0.03 | 2.9 | 0.6 |
| N(E) Red complex      |    2 |  30 |      | 33. |  3. | 0.10 | 0.03 | 2.9 | 0.8 |
| SW Faint Blue complex |    2 | 225 |      | 36. |  4. | 0.14 | 0.06 | 3.9 | 1.9 |
|-----------------------+------+-----+------+-----+-----+------+------+-----+-----|
| Outer lobe X+         |    3 | 285 | D+   | 51. | 43. | 0.19 | 0.02 | 3.7 | 3.3 |
| Outer lobe IV+        |    3 | 315 | D+   | 37. |  6. | 0.11 | 0.01 | 2.9 | 0.6 |
| NW knot ([N II])      |    2 | 310 |      | 30. |  4. | 0.12 | 0.02 | 3.6 | 1.0 |
| N(W) Red complex      |    2 | 300 |      | 32. |  3. | 0.10 | 0.01 | 3.0 | 0.6 |
| Outer lobe X-/IV-/V+  |    3 | 120 | D-   | 26. | 13. | 0.14 | 0.04 | 5.2 | 3.3 |
| SE Blue complex       |    2 | 135 |      | 42. |  6. | 0.06 | 0.03 | 1.4 | 0.8 |
| SE knot               |    2 | 135 |      |  51 |   3 | 0.09 | 0.01 | 1.7 | 0.2 |

#+name: vel-radius-plot
#+header: :var TAB=vel-radius-systems SUFF="" :colnames no
#+begin_src python :tangle ../scripts/vel-radius-systems.py 
  import numpy as np
  from astropy.table import Table
  from matplotlib import pyplot as plt
  import seaborn as sns
  ...
  figfile = f"vel-radius-systems{SUFF}.pdf"
...
#+call: vel-radius-plot(TAB=vel-radius-systems-worst, SUFF="-worst")
